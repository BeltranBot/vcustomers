  ...
  methods: {
  	...
  	updateCustomer(e) {
  		if (!this.customer.first_name || !this.customer.last_name || !this.customer.email) {
  			this.alert = {};
  			this.alert.text = 'Please fill in all required fields';
  			...
  			this.$http.put('http://slimapp/api/customers/'+this.$route.params.id, updateCustomer)
  			.then(function(response) {
  				console.log(response.body);
  				// to check for sql errors print response.body
  				this.$router.push({path: '/', query: { alert: JSON.parse(response.body) }});
  			});
  			e.preventDefault();
  		}
  		e.preventDefault();
